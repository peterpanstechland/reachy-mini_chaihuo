...
def test_synthesizer_uses_track_motif() -> None:
    synth = BeatMotionSynthesizer(beat_interval=0.469)
    synth.on_run_start()
    beat = _beat("PEAK", 0.9)
    beat["motif"] = "nod"
    _, _, _, status = synth.synthesize(0.2, beat, 0.0)
    assert status["motif"] == "nod"
