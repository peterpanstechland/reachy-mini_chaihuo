...
def test_raw_stream_output_accepts_byte_buffer() -> None:
    audio = _make_sounddevice_io()
    audio._out_ch = 2
    mono = np.array([100, -200, 300], dtype=np.int16).tobytes()
    outdata = bytearray(3 * 2 * 2)

    audio._write_output(outdata, mono)

    expected = np.array(
        [[100, 100], [-200, -200], [300, 300]], dtype=np.int16
    ).tobytes()
    assert bytes(outdata) == expected


def test_raw_stream_output_can_be_cleared() -> None:
    outdata = bytearray(b"not silent")
    _make_sounddevice_io()._clear_output(outdata)
