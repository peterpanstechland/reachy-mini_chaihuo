...
@pytest.mark.asyncio
async def test_external_interaction_preserves_current_conversation_loop() -> None:
    engine = ConversationEngine(Config())
    current = asyncio.current_task()
    engine._task = current

    await engine.set_external_interaction(True, state="music")
    assert engine._task is current
...
async def test_energy_listen_bypasses_wake_word_and_uses_rms_threshold() -> None:
    ...
    engine = ConversationEngine(
        Config(
            ...
            wake_engine="local",
            asr_initial_silence_timeout_s=0.2,
            voice_activity_threshold=0.006,
            vad_model_path="models/vad/not-installed.onnx",
        ),
        audio_backend=EnergyGateAudio(),  # type: ignore[arg-type]
    )
    engine._wake = object()
    engine._energy_listen_requested = True
    engine._energy_listen_active = True
    received: list[list[bytes]] = []
...
@pytest.mark.asyncio
async def test_energy_listen_request_rejects_busy_engine() -> None:
    engine = ConversationEngine(Config())
    engine._state = "speaking"
    assert await engine.request_energy_listen() is False
    assert engine._energy_listen_active is False
...
@pytest.mark.asyncio
async def test_start_beat_dance_restores_state_when_assets_missing() -> None:
    beat = _FakeBeatDance()
    beat.start = lambda: None  # type: ignore[method-assign]
    engine = ConversationEngine(
        Config(),
        audio_backend=_MusicFakeAudio(),  # type: ignore[arg-type]
        beat_dance=beat,  # type: ignore[arg-type]
    )
    reply = await engine.start_beat_dance()
    assert "缺失" in reply
    assert not engine._dance_loop_active
    assert beat.track_title == "测试歌曲"
