...
class ConversationEngine:
    ...
    def __init__(
        ...
        *,
        ...
        self._energy_listen_requested = False
        self._energy_listen_active = False
    ...
    async def request_energy_listen(self) -> bool:
        """Arm one wake-free turn that starts ASR after an energy threshold hit."""
        if (
            self._external_interaction_active
            or self._dance_loop_active
            or self._state in ("thinking", "speaking", "gesture", "dancing")
        ):
            return False
        if self._energy_listen_active:
            return True
        self._energy_listen_requested = True
        self._energy_listen_active = True
        await self._cancel_voice_listener()
        logger.info(
            "🎤 [免唤醒] 已启用一次性能量激活，阈值 RMS=%.4f",
            self.config.voice_activity_threshold,
        )
        self._emit_asr_status("免唤醒聆听中，请直接说话")
        return True

    async def set_external_interaction(
        self, active: bool, *, state: str = "gesture"
    ) -> None:
        """Pause/resume conversation while an exclusive controller owns the robot."""
        if active and state != "music" and self._bgm_active:
    ...
    async def _run_voice_turn(self) -> None:
        ...
        async with self._turn_lock:
            ...
            bgm_command = parse_bgm_command(user_text, list_bgm_tracks())
            if bgm_command is not None:
                await self._handle_bgm_command(
                    user_text,
                    bgm_command,
                    source="voice",
                    client_message_id="",
    ...
    async def _listen_for_speech(self) -> str:
        ...
        energy_listen = self._energy_listen_requested
        self._energy_listen_requested = False
        capture = self._audio.start_capture().__aiter__()
        try:
            if self._needs_wake_word() and not energy_listen:
            ...
            initial_audio = await self._wait_for_voice_activity(
                capture, energy_only=energy_listen
            )
            ...
            while time.monotonic() < deadline:
                ...
                try:
                    if next_chunk_task is None:
                        next_chunk_task = asyncio.create_task(anext(capture))
                    chunk = await asyncio.wait_for(
                        asyncio.shield(next_chunk_task), timeout=remaining
                    )
    ...
    async def _wait_for_voice_activity(
        self, capture: Any, *, energy_only: bool = False
    ) -> list[bytes]:
        ...
        try:
            while time.monotonic() < deadline:
                ...
                if energy_ready and (
                    ...
                    or not frontend.vad.available
                ):
                    legacy_voice_frames += 1
                else:
                    legacy_voice_frames = 0
                detected = (
                    legacy_voice_frames >= 2
                    if energy_only
                    else frame.speech or legacy_voice_frames >= 2
                )
                if detected:
                    logger.info(
                        "🎤 [%s] 检测到语音 RMS=%.4f threshold=%.4f SNR=%.1fdB P=%.2f",
                        "免唤醒能量" if energy_only else "本地 VAD",
                        frame.rms,
                        energy_trigger_threshold,
                        frame.snr_db,
    ...
    async def play_local_speech(self, pcm: bytes, sample_rate: int) -> None:
        """Play pre-generated speech through the normal motion/audio pipeline."""
        if self._audio is None:
            raise RuntimeError("音频后端尚未初始化")
        if self._dance_loop_active:
            await self.stop_beat_dance()
        async with self._speaking_scope():
            await self._play_tts_audio(pcm, sample_rate)
    ...
    def runtime_status(self) -> dict[str, object]:
        ...
        if self._beat_dance is not None:
            dance_status["dance_track_title"] = getattr(
                self._beat_dance, "track_title", "大东北我的家乡"
            )
            if self._dance_loop_active:
        ...
        return {
            **dance_status,
            ...
            "energy_listen_active": self._energy_listen_active,
            "bgm_active": self._bgm_active,
            "bgm_track_title": self._bgm_track_title,
    ...
    async def process_text(
        ...
        *,
        ...
        bgm_command = parse_bgm_command(normalized, list_bgm_tracks())
        if bgm_command is not None:
            async with self._turn_lock:
                await self._cancel_voice_listener()
                return await self._handle_bgm_command(
                    normalized,
                    bgm_command,
                    source=source,
                    client_message_id=client_message_id,
    ...
    async def _handle_bgm_command(
        self,
        text: str,
        command: tuple[str, BgmTrack | None],
        *,
        source: str,
        client_message_id: str,
        speak: bool,
        ...
        if action == "stop":
            reply = await self.stop_bgm()
        else:
            tracks = list_bgm_tracks()
            track = selected or (tracks[0] if tracks else None)
            if track is None:
                reply = "BGM 文件目前不是可播放格式，请放入标准音频并先执行转换。"
                if speak and self._audio is not None:
                    await self._speak_reply(reply)
            else:
                reply = await self.start_bgm(track.id)
        self._emit_turn_event(
            "turn_final",
            text=reply,
            sources=[],
        ...
        self._emit_turn_event("turn_status", status="done")
        self._last_activity = time.monotonic()
        return {
            "reply": reply,
            "emotion": "",
    ...
    async def start_beat_dance(self) -> str:
        ...
        self._dance_loop_active = True
        self._set_state("dancing")
        track_title = getattr(self._beat_dance, "track_title", "大东北我的家乡")
        self._beat_music_task = asyncio.create_task(
            self._play_beat_music((sr, pcm), track_title=track_title)
        )
        logger.info("🎵 歌曲舞蹈开始：%s（语音挂起）", track_title)
        return f"开始跳舞！正在播放《{track_title}》"
    ...
    def bgm_tracks(self) -> list[dict[str, str]]:
        return [track.to_dict() for track in list_bgm_tracks()]

    async def start_bgm(self, track_id: str) -> str:
        """Play one converted local BGM track and pause conversation."""
        from chaihuo_reachy.music import load_audio

        if self._external_interaction_active and not self._bgm_active:
            return "当前有其他交互正在运行，请先结束后再播放音乐。"
        track = resolve_bgm_track(track_id)
        if track is None:
            return "没有找到这首可播放的音乐。"
        info = await asyncio.to_thread(load_audio, track.path, 24_000)
        if info is None:
            return f"《{track.title}》无法解码，请重新转换音频。"
        await self.stop_bgm(resume_conversation=False)
        await self.set_external_interaction(True, state="music")
        self._bgm_active = True
        self._bgm_track_title = track.title
        if self._motion is not None:
            self._bgm_motion_task = asyncio.create_task(
                self._motion.music_gesture(),
                name="local-bgm-motion",
            )
        self._bgm_task = asyncio.create_task(
            self._play_bgm_once(info, track_title=track.title),
            name="local-bgm",
        )
        logger.info("🎵 BGM 开始播放：%s", track.title)
        return f"正在播放《{track.title}》。"

    async def stop_bgm(self, *, resume_conversation: bool = True) -> str:
        """Stop local BGM immediately and optionally resume conversation."""
        was_active = self._bgm_active or self._bgm_task is not None
        ...
        if task is not None and task is not asyncio.current_task():
            ...
            try:
                ...
                pass
        if was_active or self._bgm_motion_task is not None:
            await self._stop_bgm_motion()
        if was_active and resume_conversation and self._external_interaction_active:
            await self.set_external_interaction(False)
        if was_active:
            logger.info("⏹ BGM 已停止，恢复语音对话")
            return "音乐已停止。"
        return "当前没有播放音乐。"

    async def _stop_bgm_motion(self) -> None:
        task, self._bgm_motion_task = self._bgm_motion_task, None
        if task is not None and task is not asyncio.current_task():
            task.cancel()
            try:
                await task
                ...
                logger.exception("音乐动作任务异常")
        if self._motion is not None:
            try:
                await self._motion.reset_ready_pose(duration=0.8)
            except Exception:
                logger.exception("音乐结束姿态复位失败")

    async def _play_bgm_once(
        self, info: tuple[int, bytes], *, track_title: str
    ) -> None:
        assert self._audio is not None
        sr, frames = info
        current_task = asyncio.current_task()
        setter = getattr(self._audio, "set_output_sample_rate", None)
        if callable(setter):
            setter(sr)
        chunk_size = sr * 2
        try:
            for start in range(0, len(frames), chunk_size):
                ...
                if start >= chunk_size:
                    await asyncio.sleep(1.0)
            self._audio.mark_playback_done()
            while bool(getattr(self._audio, "is_playing", False)):
                await asyncio.sleep(0.1)
        except asyncio.CancelledError:
            raise
        finally:
            if self._bgm_task is current_task:
                self._bgm_task = None
                self._bgm_active = False
                self._bgm_track_title = ""
                await self._stop_bgm_motion()
                if self._external_interaction_active:
                    await self.set_external_interaction(False)
                logger.info("🎵 BGM 播放结束：%s", track_title)

    async def _play_beat_music(
        self, info: tuple[int, bytes], *, track_title: str = "节拍音乐"
    ) -> None:
        """Feed the selected song PCM to the speaker in a real-time loop.
