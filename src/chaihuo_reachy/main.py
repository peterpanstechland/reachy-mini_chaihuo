...
class _MJPEGStream:
    ...
    def _capture_loop(self) -> None:
        ...
        while self._running:
            ...
            if ret and frame is not None:
                frame = self._fit_for_stream(frame)
    ...
    def _fit_for_stream(self, frame: np.ndarray) -> np.ndarray:
        """Resize a frame to fit the web bounds without changing aspect ratio."""
        height, width = frame.shape[:2]
        scale = min(self._width / width, self._height / height, 1.0)
        if scale >= 1.0:
            return frame
    ...
    def get_frame_packet(self) -> tuple[int, bytes]:
        """Return the current sequence number and JPEG frame atomically."""
        with self._lock:
...
async def generate_mjpeg(stream: _MJPEGStream):
    ...
    while stream.is_running:
        sequence, frame = stream.get_frame_packet()
...
async def run_dashboard(
    ...
    *,
    ...
    from chaihuo_reachy.motion import OPENING_TALK_MOTION_GAIN
    ...
    mjpeg = _MJPEGStream(
        ...
        fps=max(24, int(round(cfg.gesture_inference_fps))),
    )
    ...
    async def _run_opening_show(audio: OpeningAudio, spec: LocalShowSpec) -> None:
        ...
        try:
            ...
            if gesture_task is not None:
                gesture_task.cancel()
                await asyncio.gather(gesture_task, return_exceptions=True)
    ...
    async def ws_endpoint(ws: WebSocket) -> None:
        ...
        def _snapshot() -> list[dict[str, Any]]:
            ...
            return [
                ...
                {"type": "show_content_status", "busy": show_content_busy},
                {
                    ...
                    "track_title": engine._bgm_track_title,
                    "tracks": engine.bgm_tracks(),
                },
        ...
        async def _handle_control(client: WebSocket, data: dict[str, Any]) -> None:
            ...
            if show_spec is not None:
                ...
                if motion is None:
                    await client.send_json(
                        {"type": "error", "message": "机器人动作控制器不可用"}
                    )
                    return
                try:
            ...
            if event_type == "get_volume":
            ...
            elif event_type == "energy_listen":
                accepted = await engine.request_energy_listen()
                if not accepted:
                    await client.send_json(
                        {
                            "type": "error",
                            "message": "当前正在回答、跳舞或手势交互，暂时不能开始免唤醒聆听",
                        }
                    )
                    return
                broadcast(
                    {
                        "type": "energy_listen",
                        "active": True,
                        "threshold": cfg.voice_activity_threshold,
                    }
                )
                ...
                broadcast(status)
                if action == "play" and not engine._bgm_active:
                    ...
                    broadcast(
                        {
                            ...
                            "active": started,
                            "track_title": getattr(
                                beat_dance, "track_title", "大东北我的家乡"
                            ),
                        }
                    )
