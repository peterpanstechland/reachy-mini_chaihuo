...
TRACK_MOTIFS: dict[str, tuple[float, float, float]] = {
    "sway": (1.0, 0.35, 0.45),
    "nod": (0.35, 1.0, 0.45),
    "roll": (0.35, 0.45, 1.0),
    "yaw": (1.0, 0.55, 0.35),
...
class BeatMotionSynthesizer:
    ...
    def synthesize(
        ...
        motif = str(beat.get("motif") or "")
        w_yaw, w_pitch, w_roll = TRACK_MOTIFS.get(motif, GROUP_MOTIFS[group])
...
class BeatDanceController:
    ...
    def __init__(self, reachy: Any, cfg: Config) -> None:
        ...
        self._track_title: str = self._music_path.stem
    ...
    def load_timeline(self) -> bool:
        ...
        self._track_title = str(
            self._timeline.get("track_title") or self._music_path.stem
        )
