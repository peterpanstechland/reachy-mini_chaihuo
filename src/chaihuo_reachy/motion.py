...
OPENING_TALK_MOTION_GAIN = 0.35
OPENING_ANTENNA_LIMIT_RAD = 0.65
OPENING_BODY_YAW_LIMIT_RAD = 0.04
OPENING_GESTURE_SEQUENCE = (
    ([-0.55, 0.55], 0.035),
    ([-0.30, 0.65], -0.04),
    ([-0.65, 0.30], 0.04),
    ([-0.40, 0.40], -0.03),
...
MUSIC_ANTENNA_LIMIT_RAD = 1.35
MUSIC_BODY_YAW_LIMIT_RAD = 0.06
MUSIC_HEAD_ROLL_LIMIT_DEG = 7.0

MUSIC_HEAD_LEFT = np.eye(4)
_music_roll = np.radians(MUSIC_HEAD_ROLL_LIMIT_DEG)
MUSIC_HEAD_LEFT[:3, :3] = np.array(
    [
        [1.0, 0.0, 0.0],
        [0.0, np.cos(_music_roll), -np.sin(_music_roll)],
        [0.0, np.sin(_music_roll), np.cos(_music_roll)],
    ]
)
MUSIC_HEAD_RIGHT = MUSIC_HEAD_LEFT.copy()
MUSIC_HEAD_RIGHT[:3, :3] = MUSIC_HEAD_LEFT[:3, :3].T
MUSIC_GESTURE_SEQUENCE = (
    (MUSIC_HEAD_LEFT, [-1.25, 0.65], 0.06),
    (HEAD_NEUTRAL, [-0.55, 1.35], 0.015),
    (MUSIC_HEAD_RIGHT, [-0.65, 1.25], -0.06),
    (HEAD_NEUTRAL, [-1.35, 1.35], -0.015),
)
...
class MotionController:
    ...
    async def opening_gesture(self, duration_s: float) -> None:
        """Run a restrained antenna/body sequence while the opening plays."""
        started = time.monotonic()
        async with self._lock:
            logger.info("🎙 开场动作开始：轻微天线与身体摆动")
            try:
                while time.monotonic() - started < max(0.0, duration_s - 1.0):
    ...
    async def music_gesture(self) -> None:
        """Loop a visible but restrained head/body/antenna sequence."""
        async with self._lock:
            logger.info("🎶 音乐动作开始：头部、身体与天线明显摆动")
            while True:
                for head, antennas, body_yaw in MUSIC_GESTURE_SEQUENCE:
                    await self._safe_goto(
                        head=head,
                        antennas=antennas,
                        body_yaw=body_yaw,
    ...
    async def reset_ready_pose(self, duration: float = 0.8) -> None:
        """Clear speech offsets, then restore the startup-ready pose."""
        self.stop_talk_motion(immediate=True)
        thread = self._talk_motion_thread
        if thread is not None and thread.is_alive():
            await asyncio.to_thread(thread.join, 0.5)
        logger.info("🤖 动作结束：复位头部、身体和天线")
        await self.return_to_neutral(duration=duration)
    ...
    def set_talk_motion_gain(self, gain: float) -> float:
        """Set speech-motion amplitude and return the previous gain."""
        with self._talk_motion_lock:
            previous = self._speech_motion.master_gain
            self._speech_motion.master_gain = max(0.0, min(1.5, float(gain)))
        return previous
