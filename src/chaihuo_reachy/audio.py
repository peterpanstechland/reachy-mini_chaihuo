...
class DuplexAudioIO:
    ...
    def _write_output(self, outdata, mono: bytes) -> None:
        ...
        if not isinstance(outdata, np.ndarray):
            if self._out_ch == 1:
                payload = mono
            else:
                samples = np.frombuffer(mono, dtype=np.int16)
                payload = np.repeat(
                    samples[:, None], self._out_ch, axis=1
                ).tobytes()
            size = min(len(payload), len(outdata))
            outdata[:size] = payload[:size]
            if size < len(outdata):
    ...
    @staticmethod
    def _clear_output(outdata) -> None:
        """Silence NumPy and RawStream/CFFI output buffers."""
